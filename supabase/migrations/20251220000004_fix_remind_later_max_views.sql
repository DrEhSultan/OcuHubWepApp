-- Migration: Fix Remind Later - Complete Fix
-- Date: 2025-12-20
-- Purpose: When user clicks "Remind Later" on an announcement with dismissible_mode = 'remind_later':
--   1. The announcement should show remind_later_count more times
--   2. With remind_later_sessions interval between each
--   3. Then STOP permanently - IGNORE max_times_seen_per_user and normal repeat mode
--
-- ROOT CAUSE IDENTIFIED:
-- 1. record_announcement_impression changes status from 'deferred' to 'seen'
-- 2. This resets the deferred state, so next "Remind Later" click starts fresh with defer_count=1
--
-- FIXES:
-- 1. record_announcement_impression: Preserve 'deferred' status (don't change to 'seen')
-- 2. get_eligible_announcements: Skip max_views check when in remind_later mode
-- 3. get_eligible_announcements: Stop showing when defer_count >= remind_later_count

-- ============================================================================
-- STEP 1: Fix record_announcement_impression
-- When a deferred announcement is shown, we need to block it from showing again
-- until user clicks "Remind Later". We do this by setting defer_until_session
-- to a high value that will be overwritten when user clicks Remind Later.
-- ============================================================================
...
BEGIN
    ...
    ON CONFLICT (announcement_id, user_id) DO UPDATE SET
        -- Preserve 'deferred', 'dismissed', 'completed' status
        status = CASE 
            WHEN user_announcement_state.status IN ('dismissed', 'completed', 'deferred') THEN user_announcement_state.status
            ELSE 'seen'
        END,
        ...
        -- CRITICAL: When showing a deferred announcement, set defer_until_session very high
        -- This blocks it from showing again until user clicks "Remind Later"
        -- which will set a proper defer_until_session value
        defer_until_session = CASE 
            WHEN user_announcement_state.status = 'deferred' THEN 2147483647
...
BEGIN
    ...
    WHERE 
        ...
        AND (
            ...
            -- BUT: For remind_later mode with defer_count > 0, don't use normal repeat logic
            OR (
                uas.status = 'seen'
                AND COALESCE(a.dismissible_mode, 'yes') != 'remind_later'  -- Not remind_later mode
                AND (
        ...
        -- IMPORTANT: Skip max_views check when in remind_later mode (defer_count > 0)
        -- In remind_later mode, the defer_count check handles the limit
        AND (
            p_surface = 'inbox'
            -- Skip max_views for remind_later mode - defer_count handles the limit
            OR (a.dismissible_mode = 'remind_later' AND COALESCE(uas.defer_count, 0) > 0)
        ...
        -- REMIND LATER EXHAUSTED CHECK (not for inbox)
        -- When defer_count >= remind_later_count, the remind later sequence is exhausted
        -- The announcement should NOT show again
        AND (
            p_surface = 'inbox'
...
-- ============================================================================
-- 
-- PROBLEM: After clicking "Remind Later", the announcement kept showing every
-- session instead of following the remind_later_count limit.
--
-- ROOT CAUSE: The eligibility logic was using normal repeat mode for 'seen' 
-- status even when dismissible_mode = 'remind_later'. This caused the 
-- announcement to show based on repeat_session_interval instead of waiting
-- for user to click "Remind Later" again.
--
-- FIX 1: get_eligible_announcements - ELIGIBILITY LOGIC
-- - For remind_later mode: Don't use normal repeat logic when defer_count > 0
-- - Only show deferred announcements when session >= defer_until_session
-- - Each "Remind Later" click sets new defer_until_session
--
-- FIX 2: get_eligible_announcements - MAX VIEWS CHECK  
-- - Skip max_times_seen_per_user when defer_count > 0
-- - The remind_later_count becomes the ONLY limit
--
-- FIX 3: get_eligible_announcements - EXHAUSTED CHECK
-- - When defer_count >= remind_later_count, stop showing completely
--
-- Expected flow with remind_later_count=3, remind_later_sessions=1:
-- 1. Session 3: User sees announcement, clicks "Remind Later"
--    → defer_count=1, defer_until_session=4, status='deferred'
-- 2. Session 4: Announcement shows (session >= defer_until_session)
--    → User clicks "Remind Later" → defer_count=2, defer_until_session=5
-- 3. Session 5: Announcement shows
--    → User clicks "Remind Later" → defer_count=3, defer_until_session=6
-- 4. Session 6+: Announcement does NOT show (defer_count=3 >= remind_later_count=3)
--
-- ============================================================================
