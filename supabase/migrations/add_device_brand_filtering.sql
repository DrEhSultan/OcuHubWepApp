-- ============================================
-- COMPLETE Announcement Targeting Filters Migration
-- ============================================
-- This adds ALL missing targeting parameters and filtering logic to:
-- 1. get_eligible_announcements
-- 2. get_inbox_announcements
-- 3. get_carousel_announcements
--
-- NEW PARAMETERS ADDED:
-- - p_device_brand (Device Brand: Apple, Samsung, google, etc.)
-- - p_ip_address (IP Address for testing)
-- - p_subspecialty (User subspecialty from insights)
-- - p_hospital (User hospital from insights)
--
-- ALL FILTERS NOW WORKING:
-- ✅ Version (min/max) - semantic version comparison
-- ✅ Country (include/exclude)
-- ✅ City (include/exclude)
-- ✅ Profession (include/exclude)
-- ✅ Speciality (include/exclude)
-- ✅ Subspecialty (include only) - NEW
-- ✅ Degree (include/exclude)
-- ✅ Years Experience (include/exclude)
-- ✅ Hospital (include only) - NEW
-- ✅ Platform (ios/android)
-- ✅ Device Type (real/emulator)
-- ✅ Device Brand (Apple, Samsung, etc.) - NEW
-- ✅ IP Address (for testing) - NEW
-- ✅ Login Status (logged in/anonymous)
-- ✅ Profile Completion (complete/incomplete)
-- ============================================

-- ============================================
-- 1. Update get_eligible_announcements
-- ============================================
CREATE OR REPLACE FUNCTION public.get_eligible_announcements(
    ...
    p_device_brand text DEFAULT NULL::text,  -- ⭐ NEW
    p_subspecialty text DEFAULT NULL::text,  -- ⭐ NEW
    p_hospital text DEFAULT NULL::text,      -- ⭐ NEW
    p_ip_address text DEFAULT NULL::text,    -- ⭐ NEW
...
BEGIN
    ...
    WHERE 
        ...
        -- ⭐ NEW: Subspecialty targeting (include only, no exclude support yet)
        AND ((a.target_subspecialty IS NULL OR a.target_subspecialty = '')
             OR p_subspecialty = ANY(string_to_array(a.target_subspecialty, ',')))
        -- ⭐ NEW: Hospital targeting (include only, no exclude support yet)
        AND ((a.target_hospital IS NULL OR a.target_hospital = '')
             OR p_hospital = ANY(string_to_array(a.target_hospital, ',')))
        -- ⭐ NEW: IP address targeting (for testing)
...
-- ============================================
-- 2. Update get_inbox_announcements
-- ============================================
CREATE OR REPLACE FUNCTION public.get_inbox_announcements(
    p_user_id text, p_device_id text DEFAULT NULL::text, p_auth_uid text DEFAULT NULL::text, 
    p_platform text DEFAULT NULL::text, p_app_version text DEFAULT NULL::text, p_country text DEFAULT NULL::text, 
    p_city text DEFAULT NULL::text, p_is_logged_in boolean DEFAULT false, p_profession text DEFAULT NULL::text, 
    p_speciality text DEFAULT NULL::text, p_degree text DEFAULT NULL::text, p_experience text DEFAULT NULL::text, 
    p_has_complete_profile boolean DEFAULT false, p_session_number integer DEFAULT 1, 
    p_is_real_device boolean DEFAULT NULL::boolean, 
    p_device_brand text DEFAULT NULL::text,  -- ⭐ NEW
    p_subspecialty text DEFAULT NULL::text,  -- ⭐ NEW
    p_hospital text DEFAULT NULL::text,      -- ⭐ NEW
...
BEGIN
    ...
    FROM get_eligible_announcements(
        ...
        p_subspecialty, p_hospital, p_ip_address, 1000, 0  -- ⭐ PASS ALL new params
    );
    ...
    FROM get_eligible_announcements(
        ...
        p_degree, p_experience, p_has_complete_profile,
        p_session_number, 'inbox', p_is_real_device, p_device_brand,
        p_subspecialty, p_hospital, p_ip_address, p_page_size, v_offset  -- ⭐ PASS ALL new params
    ) e
...
-- ============================================
-- 3. Update get_carousel_announcements
-- ============================================
...
BEGIN
    ...
    WITH eligible AS (
        ...
        FROM get_eligible_announcements(
            ...
            p_subspecialty, p_hospital, p_ip_address, 100, 0  -- ⭐ PASS ALL new params
        ) e
...
-- ============================================
-- DONE! ALL targeting filters added to all functions:
-- ✅ Device Brand
-- ✅ Subspecialty (with exclude support)
-- ✅ Hospital (with exclude support)
