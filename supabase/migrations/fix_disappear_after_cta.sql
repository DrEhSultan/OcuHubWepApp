-- Fix get_eligible_announcements to respect disappear_after_cta setting
-- When disappear_after_cta = false (Keep showing), announcement should continue to show even after CTA is clicked
...
BEGIN
    ...
    WHERE 
        ...
        -- Eligibility logic
        AND (
            -- Inbox shows ALL eligible announcements (including seen/completed)
            p_surface = 'inbox'
            -- For other surfaces (carousel, modal), apply eligibility rules
            OR uas.status IS NULL
            OR uas.status = 'eligible'
            OR uas.status = 'seen'
            -- For surveys/quizzes with remind_later, show if not completed
            OR (a.kind IN ('survey', 'quiz', 'user_insights') AND a.dismissible_mode = 'remind_later' AND uas.status != 'completed')
            -- CRITICAL: disappear_after_cta logic for non-inbox surfaces
            -- If disappear_after_cta = false (Keep showing), show even if completed
            OR (p_surface != 'inbox' AND a.disappear_after_cta = FALSE)
        )
