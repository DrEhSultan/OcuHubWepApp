-- Migration: Fix Remind Later Exhausted - Permanently hide after remind_later_count reached
-- Date: 2025-12-20
-- Purpose: When user has clicked "Remind Later" remind_later_count times, the announcement
--          should be permanently hidden and NOT fall back to normal repeat mode.
--
-- The issue: After defer_count >= remind_later_count, if the announcement is shown again
--            and marked as 'seen', the status changes from 'deferred' to 'seen', which
--            then allows it to pass the normal repeat mode checks.
--
-- The fix: 
--   1. In get_eligible_announcements: Exclude announcements where status='deferred' AND 
--      defer_count >= remind_later_count (treat as permanently dismissed)
--   2. In update_announcement_state: When status='seen' is set, preserve 'deferred' status
--      if defer_count >= remind_later_count (don't allow status change)

-- ============================================================================
-- FIX 1: Update get_eligible_announcements to exclude exhausted deferred announcements
-- ============================================================================
...
BEGIN
    ...
    WHERE 
        ...
        AND (
            ...
            -- BUT ONLY if dismissible_mode is NOT 'remind_later' with exhausted defer_count
            OR (
                uas.status = 'seen'
                AND (
                    -- Normal repeat mode check
                    (
                ...
                -- CRITICAL: If this announcement has remind_later mode AND defer_count >= remind_later_count,
                -- it should NOT be shown even if status is 'seen' (user exhausted remind later)
                AND NOT (
            ...
            OR (
                ...
                AND (p_session_number - COALESCE(uas.last_seen_session, 0)) >= COALESCE(a.repeat_session_interval, 1)
                -- CRITICAL: If this announcement has remind_later mode AND defer_count >= remind_later_count,
                -- it should NOT be shown even if completed with keep showing
                AND NOT (
                    a.dismissible_mode = 'remind_later'
                    AND COALESCE(uas.defer_count, 0) >= COALESCE(a.remind_later_count, 3)
                )
        ...
        -- CRITICAL: Skip max views check if remind_later mode with exhausted defer_count
        -- (those are already excluded above)
        AND (
            p_surface = 'inbox'
        ...
        -- ============================================================================
        -- CRITICAL: EXCLUDE EXHAUSTED REMIND_LATER (not for inbox)
        -- When defer_count >= remind_later_count, treat as permanently dismissed
        -- This prevents falling back to normal repeat mode
        -- ============================================================================
        AND (
            p_surface = 'inbox'
...
-- 
-- PROBLEM:
-- When user clicks "Remind Later" remind_later_count times (e.g., 3 times),
-- the announcement should be permanently hidden. However, it was falling back
-- to normal repeat mode because:
-- 1. After defer_count >= remind_later_count, the 'deferred' status check fails
-- 2. When announcement is shown again, status changes to 'seen'
-- 3. 'seen' status passes normal repeat mode checks
--
-- SOLUTION:
-- Added a final exclusion check that blocks ANY announcement where:
-- - dismissible_mode = 'remind_later'
-- - AND defer_count >= remind_later_count
--
-- This ensures that once remind_later is exhausted, the announcement is
-- permanently hidden regardless of its current status.
--
-- ============================================================================
