-- Migration: Add first_view_session_delay to get_eligible_announcements function
-- Date: 2025-12-19
-- Purpose: Filter announcements based on first view session delay

-- Drop existing function (required when changing return type or logic)
...
-- Recreate function with first_view_session_delay logic
CREATE OR REPLACE FUNCTION public.get_eligible_announcements(
    ...
    first_view_session_delay integer  -- NEW: Return this field
)
...
BEGIN
    ...
    SELECT 
        ...
        a.display_sequence,
        a.disappear_after_cta,
        COALESCE(a.first_view_session_delay, 0) AS first_view_session_delay  -- NEW: Return field
    ...
    WHERE 
        ...
        AND (
            ...
            OR (
                ...
                AND p_country != ALL(string_to_array(a.target_country, ','))
            )
        )
        ...
        AND (
            ...
            OR (
                ...
                AND p_city != ALL(string_to_array(a.target_city, ','))
            )
        )
        ...
        AND (
            ...
            OR (
                ...
                AND p_profession != ALL(string_to_array(a.target_profession, ','))
            )
        )
        ...
        AND (
            ...
            OR (
                ...
                AND p_speciality != ALL(string_to_array(a.target_speciality, ','))
            )
        )
        ...
        AND (
            ...
            OR a.target_degree = '' 
            OR (
                a.target_degree_exclude = FALSE 
                AND p_degree = ANY(string_to_array(a.target_degree, ','))
            )
            OR (
                a.target_degree_exclude = TRUE 
                AND p_degree != ALL(string_to_array(a.target_degree, ','))
            )
        )
        ...
        AND (
            ...
            OR (
                a.target_experience_exclude = FALSE 
                AND p_experience = ANY(string_to_array(a.target_years_experience, ','))
            )
            OR (
                a.target_experience_exclude = TRUE 
                AND p_experience != ALL(string_to_array(a.target_years_experience, ','))
            )
        )
        
        -- Platform targeting
        AND (a.target_platform IS NULL OR a.target_platform = '' OR a.target_platform = p_platform)
        
        -- NEW: First view session delay check
        -- Only applies if user has never seen this announcement (uas.status IS NULL)
        -- If first_view_session_delay > 0, check if enough sessions have passed
        AND (
            uas.status IS NOT NULL  -- User has seen it before, delay doesn't apply
            OR COALESCE(a.first_view_session_delay, 0) = 0  -- No delay set
            OR p_session_number >= COALESCE(a.first_view_session_delay, 0)  -- Enough sessions passed
        )
